...
const Profile = ({user,repos}) => {
    ...
    return ( <>{year && 
        <div className="rounded rounded-2xl flex md:flex-row  gap-x-2 flex-col md:items-stretch items-center  bg-lmText p-3 text-white">
            <div className="col-1 px-3 space-x-2">
                 <img src={user.avatar_url} alt="Profile img" className=" min-w-fit rounded-full block h-20 w-20" />
            </div>
            <div className="col-2  rounded flex flex-col  px-5 pb-3 w-full  rounded-1xl ">
                <div className="flex flex-col sm:flex-row mb-3">
                    <div className="w-full">
                        <h2 className="mb-1 text-center sm:text-left  xs:mb-3">{user.name}</h2>
                        <h4 className="mb-1 text-center    sm:text-left  xs:mb-3">{user.login}</h4>
                        <p className="font-light  sm:text-left  text-center  mb-3" >{user.bio}</p>
                    </div>

                    <h3 className="w-full  text-center xs:text-right text-white text-sm">
                        Joined {year[0]}
                    </h3>
                </div>
             ...
             {year && <div className="mt-3 bg-lmText rounded rounded-2xl text-white p-3 space-x-0">
                    <h2 className="text-2xl text-white mb-3">Newest Repos</h2>
                    <div className="d-flex flex-col ">
                        {repos && repos.map((repo)=>{
                            return <div key={repo.id} className="flex xs:gap-5 items-center gap-2 mb-3 content-between justify-between">
                                <p className="text-start  text-xs sm:text-sm  md:text-base w-full"><a href={repo.clone_url} title="Click to visit" target="_blank">{repo.name}</a></p>
